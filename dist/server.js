...
const USER_AGENT = `intodns-mcp/${exports.VERSION}`;
const REQUEST_TIMEOUT_MS = Math.min(120_000, Math.max(1_000, Number(process.env.INTODNS_REQUEST_TIMEOUT_MS) || 60_000));
...
const domainSchema = zod_1.z
    ...
    .toLowerCase()
    .max(253)
    .refine((domain) => /^(?:[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?\.)+[a-z]{2,63}$/.test(domain), "Provide a valid DNS domain name such as example.com")
    .describe("Domain name only, e.g. example.com (no URL, path, or port)");
const dkimSelectorSchema = zod_1.z
    .string()
    .trim()
    .max(253)
    .refine((selector) => selector.split(".").every((label) => label.length > 0
    && label.length <= 63
    && /^[a-z0-9_](?:[a-z0-9_-]*[a-z0-9_])?$/i.test(label)), "Provide a valid DKIM selector")
...
async function fetchWithLimits(url, init) {
...
function registerTools(server) {
    server.tool("scan_domain", "Run the fast IntoDNS.ai DNS and email security scan (~3-8s). Returns a letter grade A+ to F, numeric score 0-100, structured issue list, prioritised recommendations, full DNS/email/web/security result sections, and canonical citation URLs. Read-only — no domain mutation, no destructive side effects. The default tool for agent-visible scan evidence; use get_everything_report for a deeper single-shot report including web/blacklist/sender data, or start_deep_scan for slower Internet.nl-grade analysis. After running, use explain_issue or generate_dns_fix on any returned issue. No auth.", { domain: domainSchema }, READ_ONLY_TOOL, async ({ domain }) => jsonResponse(await apiGet("/scan/quick", { domain })));
    ...
    }, READ_ONLY_TOOL, async ({ domain, lang }) => jsonResponse(await apiGet("/scan/nis2", { domain, lang })));
    ...
    }, READ_ONLY_TOOL, async ({ domain, format }) => {
        if (format === "markdown") {
    ...
    }, ADDITIVE_TOOL, async ({ domain, format }) => {
        if (format === "markdown") {
    ...
    server.tool("get_report_snapshot", "Read a previously created IntoDNS.ai Everything Report evidence snapshot by snapshot ID. Read-only GET — returns the immutable JSON report exactly as it was at snapshot creation, with the original SHA-256 content hash and timestamp. Requires `snapshotId` from create_report_snapshot. Use to verify or re-read an audit-trail evidence record without re-running a live scan; use get_everything_report for current live data instead. No auth, fully idempotent.", {
        ...
        format: zod_1.z.enum(["json", "markdown"]).default("json").describe("Return JSON data or LLM-ready Markdown"),
    }, READ_ONLY_TOOL, async ({ snapshotId, format }) => {
    ...
    }, ADDITIVE_TOOL, async ({ domain, scanType, name }) => jsonResponse(await apiPost("/scan/deep", { domain, scanType, name })));
    server.tool("get_deep_scan_status", "Read-only status poll for a long-running Internet.nl deep scan. Returns scan progress (pending/running/finished), category scores, per-test results, and any failures. Requires a scanId returned by start_deep_scan; poll every 10-30s until status='finished'. Use after start_deep_scan; for fast single-vantage scans, prefer scan_domain. No auth, no side effects.", { scanId: zod_1.z.string().describe("Deep scan ID returned by start_deep_scan") }, READ_ONLY_TOOL, async ({ scanId }) => jsonResponse(await apiGet(`/scan/${encodeURIComponent(scanId)}`)));
    server.tool("cancel_deep_scan", "Cancel an in-progress Internet.nl deep scan. Idempotent DELETE — safe to call even if scan already finished or never started (returns acknowledgement either way). Requires `scanId` returned by start_deep_scan. Use when the user changes their mind mid-scan or when polling get_deep_scan_status would otherwise time out. No auth, no side effects beyond freeing the upstream job slot.", { scanId: zod_1.z.string().describe("Deep scan ID returned by start_deep_scan") }, DESTRUCTIVE_IDEMPOTENT_TOOL, async ({ scanId }) => jsonResponse(await apiDelete(`/scan/${encodeURIComponent(scanId)}`)));
    ...
    }, READ_ONLY_TOOL, async ({ domain, type, types }) => {
        const selectedTypes = types?.length ? types.join(",") : type;
        return jsonResponse(await apiGet("/dns/lookup", { domain, types: selectedTypes }));
    });
    server.tool("validate_dnssec", "Read-only DNSSEC chain validation. Walks the DS/DNSKEY chain from root, checks signatures, algorithm strength, key rollover state, and reports any broken links or unsigned zones. Returns chain steps, algorithm grades, and a boolean `valid`. Use when a domain claims DNSSEC; use lookup_dns(type='DNSKEY') for raw key data only. Single HTTP GET, no auth, no destructive actions.", { domain: domainSchema }, READ_ONLY_TOOL, async ({ domain }) => jsonResponse(await apiGet("/dns/dnssec", { domain })));
    server.tool("check_dns_propagation", "Compare DNS responses across the nine currently configured public and authoritative resolvers to detect propagation lag, missing answers, or inconsistent TTL/data values. Defaults to record type A and region 'all'. Returns every resolver response plus a propagation percentage and explicit inconsistency list. Use when records were just changed and you suspect staleness; for a single DNS-over-HTTPS lookup use lookup_dns instead. Read-only HTTP, no auth, and no destructive actions.", {
        domain: domainSchema,
    ...
    server.tool("check_sender_requirements", "Read-only domain-side preflight against Google/Yahoo bulk-sender requirements. Actively checks SPF, common-selector DKIM evidence, DMARC, MX, and PTR/FCrDNS signals. TLS use, one-click unsubscribe, complaint rate, and From-header behavior require a real sent message/provider telemetry and are returned as informational follow-up items, not false passes. Returns per-requirement pass/fail/warning/info plus an explicitly limited readiness summary. Use before a campaign; use analyze_raw_email or create_email_test to verify message-level requirements. Single GET, no auth.", { domain: domainSchema }, READ_ONLY_TOOL, async ({ domain }) => jsonResponse(await apiGet("/email/sender-requirements", { domain })));
    server.tool("check_email_security", "Read-only combined email-security check covering SPF parse, DKIM selector discovery, DMARC policy validation, MX IP blacklist status across major feeds, and an aggregated 0-100 email-security score with prioritised issue list. Single call replaces sequential check_spf + discover_dkim + check_dmarc + check_blacklist for the typical case. Use for one-shot email security overview; use check_sender_requirements specifically for Google/Yahoo bulk-sender compliance, or the individual check_* tools when you need only one signal. No auth, ~3-8s.", { domain: domainSchema }, READ_ONLY_TOOL, async ({ domain }) => jsonResponse(await apiGet("/email/check", { domain })));
    server.tool("create_email_test", "Create a new IntoDNS.ai inbound email-test session. Returns a unique single-use test email address (valid 60 minutes) and a `testId` used by get_email_test or poll_email_test. This is an additive, non-idempotent POST: every call creates a fresh session but never modifies prior sessions. `language` controls result text (en/nl/de/fr, default en). Use to debug an outbound message's SPF/DKIM/DMARC, headers, and spam triggers; after sending, call poll_email_test. No auth.", { language: zod_1.z.enum(["en", "nl", "de", "fr"]).default("en").describe("Result language") }, ADDITIVE_TOOL, async ({ language }) => jsonResponse(await apiPost("/email-test/create", { language })));
    server.tool("get_email_test", "Read-only status read for an email-test session. Returns 'pending' until a test email arrives at the unique address returned by create_email_test, then full SPF/DKIM/DMARC/headers/spam-score result once processed. Requires `testId` from create_email_test. Use after sending a test message to that address; for explicit processing of just-arrived mail use poll_email_test instead. Idempotent GET, no auth.", { testId: zod_1.z.string().describe("Email test ID returned by create_email_test") }, READ_ONLY_TOOL, async ({ testId }) => jsonResponse(await apiGet(`/email-test/${encodeURIComponent(testId)}`)));
    server.tool("poll_email_test", "Process the latest received message in an email-test session. Idempotent POST: if no message has arrived yet, returns 'pending'; if a message arrived since the last call, parses it and returns full authentication + content analysis. Requires `testId` from create_email_test. Use to actively trigger parsing after the user reports sending the test mail; use get_email_test for passive status polling without processing. No auth, no destructive side effects.", { testId: zod_1.z.string().describe("Email test ID returned by create_email_test") }, ADDITIVE_IDEMPOTENT_TOOL, async ({ testId }) => jsonResponse(await apiPost(`/email-test/${encodeURIComponent(testId)}`)));
    server.tool("analyze_raw_email", "Read-only analysis of a pasted raw RFC-5322 MIME email source. Parses Authentication-Results, Received chain, SPF/DKIM/DMARC/ARC verdicts, sender IP reputation/blacklist status, content-side spam triggers (suspicious URLs, misleading From, content/HTML imbalance), and produces a 0-100 spam score plus AI-assisted fix suggestions. `rawEmail` is full headers+body, max 500KB. Use to debug a specific failing email when the user can paste the raw source from their MUA; use create_email_test instead when the user can resend it. POST body is processed in-memory and not stored. No auth.", { rawEmail: zod_1.z.string().max(500_000).describe("Raw email source including headers and body, max 500,000 characters") }, READ_ONLY_TOOL, async ({ rawEmail }) => jsonResponse(await apiPost("/email-test/analyze-raw", { rawEmail })));
    server.tool("parse_dmarc_report", "Read-only parser for a DMARC aggregate (RUA) XML report (RFC 7489). Turns the raw XML that mailbox providers send into structured JSON: report metadata (org, report id, date range), the published policy (p/sp/adkim/aspf/pct), and one row per sending source with source IP, message count, evaluated disposition (none/quarantine/reject), aligned SPF/DKIM results, and pass/fail totals. Provide the report as `xml` (raw text) or `gzipBase64` (a base64-encoded .gz attachment). Use to programmatically read DMARC reports an agent fetched from the rua@ mailbox; the report is parsed in-memory and not stored. No auth, no side effects.", {
        xml: zod_1.z.string().max(5 * 1024 * 1024).optional().describe("Raw DMARC aggregate report XML (root <feedback>), max 5 MB"),
        gzipBase64: zod_1.z.string().max(7 * 1024 * 1024).optional().describe("Base64-encoded gzip of the report (.gz attachment); used when xml is omitted"),
    }, READ_ONLY_TOOL, async ({ xml, gzipBase64 }) => jsonResponse(await apiPost("/dmarc/parse", gzipBase64 ? { gzipBase64 } : { xml })));
    server.tool("whois_lookup", "Read-only WHOIS/RDAP lookup for a domain or IP address. For domains it returns registrar, EPP domain-status codes, nameservers, registration/expiry/last-changed dates, and the abuse contact; for IPs it returns the network allocation (CIDR, name, type). Data is sourced live from the IANA RDAP bootstrap with an rdap.org fallback. Registrant personal data is usually GDPR-redacted — that is normal, not an error. Use to check domain ownership, age, or expiry, vet a suspicious domain, or find an abuse contact; for DNS records use lookup_dns instead. `query` is a domain name or an IPv4/IPv6 address. No auth, no side effects.", { query: zod_1.z.string().describe("A domain name (example.com) or an IPv4/IPv6 address") }, READ_ONLY_TOOL, async ({ query }) => jsonResponse(await apiGet("/whois", { query })));
    server.tool("check_http3", "Read-only HTTP/3 + QUIC support check for a domain. Combines three signals: Alt-Svc HTTP response header advertising h3, HTTPS/SVCB DNS records advertising alpn=\"h3\", and a live QUIC probe to UDP/443 verifying the handshake completes. Returns per-signal verdict plus an aggregate 'http3_ready' boolean. Use when validating CDN/Cloudflare HTTP/3 rollouts or auditing modern transport posture; not relevant for mail-only domains. No auth, ~2-5s due to UDP handshake timeout.", { domain: domainSchema }, READ_ONLY_TOOL, async ({ domain }) => jsonResponse(await apiGet("/http3/check", { domain })));
    ...
    }, READ_ONLY_TOOL, async ({ domain, issue, context }) => jsonResponse(await apiPost("/ai/explain", { domain, issue, context })));
    ...
    }, READ_ONLY_TOOL, async ({ domain, style }) => jsonResponse({
        domain,
        style,
    ...
    }, READ_ONLY_TOOL, async ({ file }) => textResponse(await siteGet(`/${file}`)));
    ...
    }, READ_ONLY_TOOL, async ({ url, strict }) => jsonResponse(await apiPost("/csp/scan", { url, strict })));
}
